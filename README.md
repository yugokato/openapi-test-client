...
the [SampleAppAPIClient](src/openapi_test_client/clients/sample_app) that comes with this repository as an example client. This demo client was actually 
generated by the following `openapi-client generate` command against the Quart-based [sample app](src/sample_app) 
demo application:
```
openapi-client generate -u http://127.0.0.1:5000/openapi.json -a sample_app
``` 
It was also manually extended with a custom [post-request hook](src/openapi_test_client/clients/sample_app/api/request_hooks/post_request.py) for automatically managing auth tokens 
before login and after logout.

> [!NOTE]
> If you would like to try this `sample_app` API client by yourself, follow the following steps to start the demo application before moving forward:
> 1. Clone the repo
> 2. Set up with `app` server-side extra dependency  (see the "Setup" section)
> 3. Start the application with `QUART_APP=sample_app.main:app quart run` command
> ```shell
> $ QUART_APP=sample_app.main:app quart run
> * Serving Quart app 'sample_app'
> * Debug mode: False
...
> - `SampleAppAPIClient` will automatically set the `token` value to the API session in the client's post-request logic mentioned earlier
