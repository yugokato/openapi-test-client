...
from api_client_core.types import (
    Alias,
    DataclassModel,
    DataclassModelField,
    File,
...
from api_client_core.types import EndpointModel as _EndpointModel
from api_client_core.types import Kwargs as _Kwargs
...
class EndpointModel(_EndpointModel):
    """OpenAPI endpoint model. Adds Pydantic conversion on top of the core `EndpointModel`."""

    @classmethod
    @lru_cache
    def to_pydantic(cls) -> type[PydanticModel]:
        """Convert the endpoint model to a Pydantic model for strict validation"""
...
class _QualNameReprMeta(type(BaseModel)):
    """Metaclass that renders a class by its bare qualified name, dropping the module.

    Pydantic models are dynamically generated by `to_pydantic()` and live in no real module, so the default
    `<class 'module.Name'>` repr shows a misleading path. This strips the module from the display.
    """
...
class PydanticModel(BaseModel, metaclass=_QualNameReprMeta):
