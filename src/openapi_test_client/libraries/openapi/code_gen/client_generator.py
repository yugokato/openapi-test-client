...
import openapi_test_client.libraries.openapi.types as types_module
import openapi_test_client.libraries.openapi.utils.endpoint_model as openapi_endpoint_model_util
...
from openapi_test_client.libraries.openapi.code_gen.code import diff_code, format_code, parse_code
from openapi_test_client.libraries.openapi.code_gen.constants import BACKSLASH, TAB
from openapi_test_client.libraries.openapi.code_gen.utils import (
...
if TYPE_CHECKING:
    from openapi_test_client.libraries.openapi import EndpointFunc
...
'''
    ...
    code = format_code(
        ...
        f"API_CLASSES = {base_api_class.__name__}.init()\n"
    )
...
def generate_api_class(
    ...
    if class_name:
        ...
        class_name_part = to_class_name(tag)
...
def update_endpoint_functions(
    ...
    def update_existing_endpoints(target_api_class: type[OpenAPIBase[Any]] = api_class) -> None:
        ...
        for matched in re.finditer(regex_ep_func, current_code):
            ...
            endpoint_model = openapi_endpoint_model_util.create_endpoint_model_from_spec(endpoint_function, api_spec)
...
def generate_api_client(temp_api_client: OpenAPIClient, show_generated_code: bool = True) -> type[OpenAPIClient]:
    ...
    for api_class in _get_api_classes(app_client_api_class_dir, base_api_class):
...
def _get_api_classes(api_class_dir: Path, base_api_class: type[T]) -> list[type[T]]:
    ...
    :param api_class_dir: Directory containing API class modules
    :param base_api_class: Base API class to filter subclasses by
    """
    assert api_class_dir.is_dir()
    mod_name = get_module_name_by_file_path(api_class_dir / "__init__.py").removesuffix(".__init__")
    return get_api_classes(mod_name, base_api_class)
