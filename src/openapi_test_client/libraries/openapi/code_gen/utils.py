...
from openapi_test_client.libraries.openapi.code_gen.constants import BACKSLASH, TAB
...
from openapi_test_client.libraries.openapi.utils.param_model import get_param_models, merge_models
from openapi_test_client.libraries.openapi.utils.param_type import (
    get_param_model,
    has_param_model,
    is_optional_type,
    is_union_type,
...
def generate_func_signature_code(model: type[EndpointModel]) -> str:
    ...
    for field_name, field_obj in model_dataclass_fields.items():
        is_path_var = field_obj.metadata.get("path", False)
...
def generate_imports_code_from_model(
    ...
    from openapi_test_client.libraries.openapi.code_gen.client_generator import API_MODEL_CLASS_DIR_NAME
            ...
            if typing_origin := get_origin(obj_type):
                if typing_origin is Annotated:
                    ...
                    if is_optional_type(obj_type):
                        ...
                        module_and_name_pairs.add((openapi_types_module.__name__, Optional.__name__))  # type: ignore[attr-defined]
                    [generate_imports_code(m) for m in get_args(obj_type)]
                else:
                ...
                module_name = (
                    openapi_types_module.__name__ if hasattr(openapi_types_module, name) else obj_type.__module__
                )
                module_and_name_pairs.add((module_name, name))
    ...
    if has_unset_field:
        imports_code += f"from {openapi_types_module.__name__} import Unset\n"
...
def generate_type_annotation_code(tp: Any) -> str:
    ...
    elif isinstance(tp, Query):
        return f"{type(tp).__name__}()"
