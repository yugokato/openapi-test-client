...
from openapi_test_client.libraries.core import types as core_types_module
from openapi_test_client.libraries.core.utils.endpoint_model import is_httpx_passthrough_field
from openapi_test_client.libraries.openapi import types as openapi_types_module
...
def generate_model_name(base_name: str) -> str:
    ...
    """
    model_name = to_class_name(clean_model_field_name(base_name))
...
def generate_model_name_for_dataclass_field(field_name: str, is_list: bool = False) -> str:
    ...
    """
    model_name = to_class_name(clean_model_field_name(field_name))
...
def get_reserved_model_names() -> list[str]:
    ...
    custom_param_annotation_names = [
        x.__name__
        for mod in [core_types_module, openapi_types_module]
...
def clean_model_field_name(name: str) -> str:
    ...
    name = core_endpoint_model_util.clean_model_field_name(name)
    if name in (*get_reserved_model_names(), *dir(dict), *ENDPOINT_FUNC_CONTROL_KWARGS):
        # Escape OpenAPI-layer reserved names (Format, Constraint, ParamModel, …), dict attr
        # names that would conflict with ParamModel's dict base class, and endpoint function
        # control kwargs (quiet, with_hooks, raw_options, validate, …) that are consumed by
        # the request machinery and must never be forwarded as API parameters.
        name += "_"
    return name
