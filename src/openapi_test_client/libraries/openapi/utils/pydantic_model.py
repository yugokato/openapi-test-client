...
@contextmanager
def in_validation_mode(enabled: bool = True) -> Generator[None, Any, None]:
    """Temporarily override validation mode for the duration of the context.

    :param enabled: When True, activates validation mode. When False, deactivates it even if the
                    VALIDATION_MODE env var is set. On exit, restores the original env var state.
    """
    original = os.environ.get("VALIDATION_MODE")
    os.environ["VALIDATION_MODE"] = "true" if enabled else "false"
    try:
        yield
    finally:
        if original is None:
            ...
            os.environ["VALIDATION_MODE"] = original


def to_pydantic(cls: type[DataclassModel]) -> type[PydanticModel]:
    """Convert a dataclass model to a Pydantic model for strict validation

    :param cls: The dataclass model class to convert
    """
    model_fields = {}
    for field_name, field_obj in cls.__dataclass_fields__.items():
        model_fields[field_name] = generate_pydantic_model_field(cls, field_obj)
    return cast(
        type[PydanticModel],
        create_model(
...
def _convert_annotated_type_recursively(outer_tp: Any, inner_tp: Any, default_value: EllipsisType | None) -> Any:
    if annotated_type := param_type_util.get_annotated_type(inner_tp):
        ...
        if fmt := filter_annotated_metadata(annotated_type, Format):
            base_type = convert_type_from_param_format(base_type, fmt.value)
