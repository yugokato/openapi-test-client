...
from openapi_test_client.libraries.core.endpoints.utils import param_type as param_type_util
from openapi_test_client.libraries.core.types import Constraint, Optional, ParamModel, PydanticModel


class TestPydanticModel:
    """Tests for Dataclass to Pydantic model conversion and Pydantic validation"""
    ...
    def test_pydantic_model_conversion(self, NewParamModel: type[ParamModel], input_value: Any) -> None:
        """Test that Pydantic model conversion and validation with correct input value type works"""
        ...
        validated_value = getattr(validated_model, field.name)
        assert validated_value == input_value
    ...
    def test_pydantic_model_validation(self, NewParamModel: type[ParamModel], input_value: Any) -> None:
        """Test that Pydantic model conversion and validation with incorrect input value type raises ValidationError"""
        field = next(iter(NewParamModel.__dataclass_fields__.values()))
        pydantic_model = NewParamModel.to_pydantic()
        assert issubclass(pydantic_model, PydanticModel)
        is_union = any(
    ...
    def test_pydantic_model_with_annotated_format(
        self, ParamModelWithParamFormats: type[ParamModel], field_name: str, value: Any
    ) -> None:
        """Test that Pydantic model conversion with annotated param format metadata works as expected"""
